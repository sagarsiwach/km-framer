  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        // Fetch vehicle data with explicit headers
        ...
        console.log(
          "BookingContainer API Response Status:",
        ...
        console.log(
          "BookingContainer API Response Text Length:",
          responseText.length
        )
        console.log(
          "BookingContainer API Response Text Preview:",
        ...
        let vehicleResult
        try {
          vehicleResult = JSON.parse(responseText)
          console.log(
            "BookingContainer Parsed Result:",
            vehicleResult
          )
        } catch (parseError) {
          console.error(
            "BookingContainer JSON Parse Error:",
            parseError
          )
          throw new Error(
        ...
        if (vehicleResult.status === "success" && vehicleResult.data) {
          setVehicleData(vehicleResult.data)

          // Initialize default selected vehicle if available
          if (
            vehicleResult.data.models &&
            vehicleResult.data.models.length > 0
          ) {
            const firstVehicle = vehicleResult.data.models[0]
            ...
            const vehiclePricing = vehicleResult.data.pricing.find(
