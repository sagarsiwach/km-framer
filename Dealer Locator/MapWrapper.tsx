...
const createMapboxPopupContent = (
 ...
 <div style="font-family: ${theme.typography.fontFamily || "Geist, sans-serif"}; font-size: 12px; color: ${theme.colors.onSurface}; max-width: 180px; padding: 8px 12px;">
    <h3 style="margin: 0 0 4px; font-size: 14px; font-weight: 500; line-height: 1.3;">${dealer.name}</h3>
    <p style="margin: 0; font-size: 12px; color: ${theme.colors.neutral[600]}; line-height: 1.3;">${dealer.address.formatted}</p>
     ${dealer.distance !== undefined && dealer.distance >= 0 ? `<p style="margin: 4px 0 0; font-size: 12px; color: ${theme.colors.neutral[600]}; font-weight: 400;">${dealer.distance} ${distanceUnit}</p>` : ""}
  </div>`
...
const MapWrapper: React.FC<MapWrapperProps> = ({
  ...
  const handleMapInteraction = useCallback(
    (e: mapboxgl.MapMouseEvent & mapboxgl.EventData) => {
      ...
      const features = mapboxMapRef.current?.queryRenderedFeatures(
        e.point,
        {
    ...
    dealers.forEach((dealer) => {
      ...
      markerEl.innerHTML = `
        ...
        ">
          <svg
            ...
            <path
              d="M17 0C7.611 0 0 7.597 0 16.966C0 27.447 14.976 46.642 15.618 47.308C16.302 48 17.698 48 18.382 47.308C19.024 46.642 34 27.447 34 16.966C34 7.597 26.389 0 17 0Z"
              fill="${isSelected ? theme.colors.primary : theme.colors.neutral[700]}"
            />
          </svg>
          <div style="
            position: absolute;
      ...
      if (existingMarker) {
        ...
        try {
          ...
          console.error(
            "Failed to create Mapbox Marker:",
            error,
            dealer
          )
        }
