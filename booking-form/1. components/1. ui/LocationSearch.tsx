...
export function getLocationIcon(locationStatus) {
  ...
  return (
    <motion.svg
      ...
      width={`${iconSize}px`}
      fill={iconColor}
      animate={locationStatus === "searching" ? { opacity: [1, 0.5, 1] } : {}}
      transition={
...
export function LocationField({
  ...
  useEffect(() => {
    if (typeof document !== "undefined") {
      ...
      let container = document.getElementById("location-dropdown-container");
      if (!container) {
    ...
    if (pincodeRegex.test(value)) {
      ...
      try {
        ...
        if (results && results.length > 0) {
          ...
          if (debugMode) console.log(`No results found for pincode: ${value}`);
